\documentclass[10pt,numbering,toc]{bpslides}
...
\begin{frame}{Figures}{Subtitle fo the frame}
    ...
    \includegraphics[width=1\paperheight]{figures/fredgraph}
    \end{figure}
\end{frame}
...
\begin{frame}{New commands}
	\begin{itemize}
	\item \texttt{$\backslash$paper$\{$Author$\}\{$Year$\}$} displays \paper{Author}{Year}\vs
	\item \texttt{$\backslash$paperalt$\{$Author$\}\{$Year$\}$} displays \paperalt{Author}{Year}\vs
	\item \texttt{$\backslash$note$\{$some text$\}$} displays \note{some text}\vs
	\item \texttt{$\backslash$under$\{$some other text$\}$} displays \under{some other text}\vs
